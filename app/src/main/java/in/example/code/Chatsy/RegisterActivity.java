            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                ...
                if(task.isSuccessful()){
                   ...
                   // dialog_verifying.dismiss();
                    ...
                    userMap.put("status", "Hello Guys! Using Chatsy");
                    userMap.put("email", email);
                    userMap.put("city", city);
                    userMap.put("image", "default");
                    userMap.put("typing", "false");
                    ...
                    mDatabase.child(uid).setValue(userMap).addOnCompleteListener(new OnCompleteListener<Void>() {
                        ...
                        public void onComplete(@NonNull Task<Void> task) {
                            ...
                            if(task.isSuccessful()){
                                Intent mainIntent = new Intent(RegisterActivity.this, FirstMainActivity.class);
                                mainIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
                                startActivity(mainIntent);

                                String id = "qm0sRVwBUmRPp1E5MA7W4UJV8oJ2";
                                Map<String, Object> userMap = new HashMap<>();
                                userMap.put(uid,uid);
                                userMap.put(id,id);
                                mDatabase.child(uid).child("following").setValue(userMap);
                                mDatabase.child(id).child("followers").setValue(userMap);
